...
export class UsersController {
  ...
  @Patch('profile') // Maneja solicitudes PATCH a /api/users/profile
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Actualiza el perfil del usuario autenticado' })
  @ApiBody({ type: UpdateProfileDto, description: 'Datos a actualizar del perfil' })
  @ApiResponse({
    status: 200,
  ...
  @UseGuards(JwtAuthGuard) // Protege este endpoint
  @Get('2fa/status') // Maneja solicitudes GET a /api/users/2fa/status
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Obtiene el estado de la autenticación de dos factores (2FA) del usuario' })
  @ApiResponse({
  ...
  @UseGuards(JwtAuthGuard) // Protege este endpoint
  @Post('2fa/activate') // Maneja solicitudes POST a /api/users/2fa/activate
  @ApiBearerAuth('access-token')
  @ApiOperation({ summary: 'Activa la autenticación de dos factores (2FA) después de verificar el código' })
  @ApiBody({
    schema: {
      type: 'object',
      properties: {
  ...
  @ApiResponse({ status: 401, description: 'No autorizado o código 2FA incorrecto' })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado' })
  async activate2FA(@Request() req, @Body('code') code: string) {
    const user = await this.usersService.findById(req.user.sub);
    if (!user || !user.totpSecret) throw new UnauthorizedException('2FA no configurado');
  ...
  @ApiResponse({ status: 401, description: 'No autorizado' })
  @ApiResponse({ status: 404, description: 'Usuario no encontrado' })
  async disable2FARequest(@Request() req) {
    const user = await this.usersService.findById(req.user.sub);
    if (!user) throw new NotFoundException('Usuario no encontrado');
